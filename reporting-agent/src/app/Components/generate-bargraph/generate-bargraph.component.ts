...
@Component({
  selector: 'app-generate-bargraph',
  ...
  templateUrl: './generate-bargraph.component.html',
  styleUrl: './generate-bargraph.component.scss',
  standalone: true
})

export class GenerateBarGraphComponent implements OnInit {
  reportBarGraphForm: FormGroup;
  ...
  constructor(private fb: FormBuilder, private graphqlService: GraphqlService, private restService: RestService) {
    this.reportBarGraphForm = this.fb.group({
      mainEntity: ['', Validators.required],
      xField: ['', Validators.required],
      yField: ['', Validators.required],
  ...
  ngOnInit(): void {
    ...
    this.reportBarGraphForm.get('mainEntity')?.valueChanges
  ...
  get mainEntity(): FormControl {
    return this.reportBarGraphForm.get('mainEntity') as FormControl;
  }

  get relatedEntityAndFields(): FormArray {
    return this.reportBarGraphForm.get('relatedEntityAndFields') as FormArray;
  }

  get andConditions(): FormArray {
    return this.reportBarGraphForm.get('andConditions') as FormArray;
  }

  get orConditions(): FormArray {
    return this.reportBarGraphForm.get('orConditions') as FormArray;
  }

  get sortConditions(): FormArray {
    return this.reportBarGraphForm.get('sortConditions') as FormArray;
  }

  //main entity
  onMainEntityChange(event: MatSelectChange) {
    const value = event.value ?? '';
    this.reportBarGraphForm.get('mainEntity')?.setValue(value);
  }

  onXLabelChange(event: MatSelectChange) {
    const value = event.value ?? '';
    this.reportBarGraphForm.get('xField')?.setValue(value);
  }

  onYLabelChange(event: MatSelectChange) {
    const value = event.value ?? '';
    this.reportBarGraphForm.get('yField')?.setValue(value);
  }
  ...
  generateBarGraph() {
    if (this.reportBarGraphForm.valid) {
      this.isLoading = true;
      this.isDownloadReady = false;
      const payload = transformToPayload(this.reportBarGraphForm.value);
      console.log(
        'Generating bar graph with:',
        transformToPayload(this.reportBarGraphForm.value)
      );
      this.restService.generateBarGraph(payload).subscribe({
        next: (response: Blob) => {
          console.log('Success');
